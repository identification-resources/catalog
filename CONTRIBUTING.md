...
| `title`      | ✔        | ✔ only if the entry is multilingual | Full title of the work | plain text, no HTML/RTF/Markdown. Subtitles delimited with full stops (`.`), or a colon (`:`) if appropriate |
...
| `fulltext_url` | ❌        | ✔ only if no URL is preferred | Full text, preferably HTML/PDF as opposed to viewer pages | [URL](https://url.spec.whatwg.org/) |
...
| `ISBN`       | ❌        | only for ISBN-10/ISBN-13 | ISBN(s) of the entry. | [ISBN](https://www.isbn-international.org/) of 13 or 10 digits, no grouping/hyphens |
...
| `part_of`    | ❌        | ✔               | Entries that this entry is listed in, corrected by or part of | `B` followed by any number of digits (no leading zero) |
